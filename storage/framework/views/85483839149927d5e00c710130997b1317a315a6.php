            ...
            <div class="row mb-2">
                <div class="col-sm-6">
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('patient-dental-histories-create')): ?>
                        <h3>
                            <a href="<?php echo e(route('patient-dental-histories.create')); ?>" class="btn btn-outline btn-info">+
                                <?php echo app('translator')->get('Add Patient Dental History'); ?>
                            </a>
                        </h3>
                    <?php endif; ?>
                </div>
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active"><?php echo app('translator')->get('Patient Dental List'); ?></li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header bg-info">
                    <h3 class="card-title"><?php echo app('translator')->get('Patient Dental History List'); ?></h3>
                    <div class="card-tools">

                        <a class="btn btn-primary float-right" target="_blank"
                            href="<?php echo e(route('patient-dental-histories.index')); ?>?export=1">
                                        ...
                                        <label><?php echo app('translator')->get('Patient'); ?></label>
                                            <select name="user_id" class="form-control select2" id="user_id">
                                                ...
                                                <?php $__currentLoopData = $patients->sortBy(fn($patient) => strtolower($patient->user->name ?? '')); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $patient): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <option value="<?php echo e($patient->id); ?>" <?php echo e(old('user_id', request()->user_id) == $patient->id ? 'selected' : ''); ?>><?php echo e($patient->name); ?></option>
                        ...
                        <tbody>
                            <?php $__currentLoopData = $patients->sortBy(fn($patient) => strtolower($patient->name ?? '')); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $patient): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php if($patient->patientDentalHistories->isNotEmpty()): ?>
                                            <?php echo e($patient->patientDentalHistories->first()->doctor->name ?? 'N/A'); ?>
                                    ...
                                    <td class="responsive-width">
                                        <?php if($patient->patientDentalHistories->isNotEmpty()): ?>
                                            <a href="<?php echo e(route('patient-dental-histories.show', $patient->patientDentalHistories->first())); ?>"
                                        ...
                                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('patient-dental-histories-update')): ?>
                                            <a href="<?php echo e(route('patient-dental-histories.edit', $patient->patientDentalHistories->first())); ?>"
                                                class="responsive-width-item btn btn-info btn-outline btn-circle btn-lg"
...
<?php echo $__env->make('layouts.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\dental\dental-main\resources\views/patient-dental-histories/index.blade.php ENDPATH**/ ?>