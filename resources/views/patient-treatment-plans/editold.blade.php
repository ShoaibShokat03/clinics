                                                ...
                                                <td class="border ">
                                                    ...
                                                    ->count();
                                                        ...
                                                        if ($patientNotesCount == 0) {
                                                            $isDisplayAddNote = 'd-none';
                                                        }
                                                    @endphp
                                                                            <button style="width:107px !important; " type="button"
                                                                                class="btn btn-sm btn-secondary show-notes-btn {{$isDisplayAddNote}}"
                                                                                data-toggle="modal"
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on("click", ".m-add", function() {
                ...
                rowTreatmentPlan.find('#notesButtons').addClass('d-none');
            ...
            $(document).on("click", ".m-save", function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // Show the notesButtons div
                        $row.find('#notesButtons').removeClass('d-none');
                     },
            ...
            function updateRowUI($row, planProcedure) {
                ...
                var $notesButtons = $row.find('#notesButtons');
                ...
                $notesButtons.addClass('d-none'); // Hide notes buttons initially
                ...
                if (planProcedure.ready_to_start === 'yes') {
                    ...
                    if (planProcedure.is_procedure_started === 'yes') {
                        ...
                        $notesButtons.removeClass('d-none'); // Show notes buttons when procedure is started
...
<script>
    $(document).ready(function() {
        ...
        $('.submit_notes_form').on('submit', function(e) {
            e.preventDefault();
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        $('#declineModal_' + response.tooth_number).modal('hide');
                        $(`button[data-procedure-id="${response.patient_treatment_plan_procedure_id}"]`).removeClass("d-none"); // Close the modal

                         // Hide the modal
                        alert('Form submitted successfully!');
                        // Reset the current form that was submitted
                        $form[0].reset();
                    } else {
    ...
    $(document).on('show.bs.modal', '.show_treatment_notes', function (e) {
        ...
        //     if (!$(this).data('tooth-number') || $(this).data('tooth-number') !== toothNumber) {
        //         $(this).remove();
        //     }
        // });
    });
...
document.querySelectorAll('.btn-primary[data-toggle="modal"]').forEach(function(button) {
    button.addEventListener('click', function() {
        var procedureId = button.getAttribute('data-procedure-id');
        var modalId = button.getAttribute('data-target');
        var modal = document.querySelector(modalId);
        if (modal) {
            var input = modal.querySelector('input[name="plan_procedure_id"]');
            if (input) {
                input.value = procedureId;
            }
        }
    });
});
...
$(document).ready(function() {
    $('.show-notes-btn').on('click', function() {
