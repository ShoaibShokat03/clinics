            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">@lang('Patient Medical List')</li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header bg-info">
                    <h3 class="card-title">@lang('Patient Medical History List')</h3>
                    <div class="card-tools">
                        <a class="btn btn-primary float-right" target="_blank"
                            href="{{ route('patient-medical-histories.index') }}?export=1">
                                            ...
                                            <select name="doctor_id" class="form-control select2" id="doctor_id">
                                                ...
                                                @foreach ($doctors->sortBy(fn($doctor) => strtolower(optional($doctor->user)->name)) as $doctor)
                                                    @if (!is_null(optional($doctor->user)->name))
                                                        <option value="{{ $doctor->user->id }}" {{ old('doctor_id', request()->doctor_id) == $doctor->user->id ? 'selected' : '' }}>{{ optional($doctor->user)->name }}</option>
                                                    @endif
                                                @endforeach
                                            </select>
                                        </div>
                        ...
                        <tbody>
                            @foreach ($patients->sortBy(fn($patient) => strtolower($patient->name ?? '')) as $patient)
                                <tr>
                                    ...
                                    <td>{{ $patient->name }}</td>
                                    <td>
                                        @if ($patient->patientMedicalHistories->isNotEmpty())
                                            {{ $patient->patientMedicalHistories->first()->doctor->name ?? 'N/A' }}
                                        @else
                                            N/A
                                        @endif
                                    </td>
                                    <td class="responsive-width">
                                        @if ($patient->patientMedicalHistories->isNotEmpty())
                                            <a href="{{ route('patient-medical-histories.show', $patient->patientMedicalHistories->first()) }}"
                                        ...
                                        @endif
                                        @if ($patient->patientMedicalHistories->isNotEmpty())
                                            <a href="{{ route('patient-medical-histories.edit', $patient->patientMedicalHistories->first()) }}"
                                                class="responsive-width-item btn btn-info btn-outline btn-circle btn-lg"
