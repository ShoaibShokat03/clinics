                        ...
                        <div class="invoice">
                            <div class="clinic-header">
                                <h4 style="font-size: 12px;margin-bottom:10px;">
                                    {{ $setting['general.company_name'] ?? 'Dental Clinic' }}</h4>
                            </div>
                                ...
                                <table>
                                    <tr>
                                        ...
                                        <td class="gender"><strong>Gender</strong>
                                            {{ $prescription->user->gender ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Date</strong>
                                            {{ date('d M Y', strtotime($prescription->prescription_date)) }}</td>
                                        <td><strong>Prescription#</strong>
                                            {{ str_pad($prescription->id, 4, '0', STR_PAD_LEFT) }}</td>
                                            ...
                                            @foreach ($items as $item)
                                                <tr>
                                                    <td style="text-align: left;">
                                                        {{ $item->ddmedicine->name ?? '-' }}
                                                    </td>
                                                    <td>
                                                        {{ $item->ddmedicinetype->name ?? '-' }}
                                                    </td>
                                                    <td>
                                                        {{ $item->instruction ?? '-' }}
                                                    </td>
                            ...
                            @if ($prescription->note)
                                <div class="row mt-custom ml-custom" style="margin-left: 0 !important;">
                                    <div class="col-12">
                                        ...
                                        <p style="font-size: 10px; margin-top: 2px;">{{ $prescription->note }}</p>
                                    </div>
                                </div>
                            @endif
