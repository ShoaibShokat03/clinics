            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">@lang('Patient Social History List')</li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-header bg-info">
                    <h3 class="card-title">@lang('Patient Social History List')</h3>
                    <div class="card-tools">
                        <a class="btn btn-primary float-right" target="_blank"
                            href="{{ route('patient-social-histories.index') }}?export=1">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="name" class="form-control select2" id="patient_id">
                                                ...
                                                @foreach ($patients->sortBy(fn($patient) => strtolower($patient->user->name ?? '')) as $patient)
                                                    <option value="{{ $patient->name }}" {{ old('name', request()->name == $patient->name ? 'selected' : '') }}>
                                                        {{ $patient->name ?? '-' }}
                                                    </option>
                        ...
                        <tbody>
                            @foreach ($patients->sortBy(fn($patient) => strtolower($patient->name ?? '')) as $patient)
                                <tr>
                                    ...
                                    <td>{{ $patient->name }}</td>
                                    <td>
                                        @if ($patient->patientSocialHistories->isNotEmpty())
                                            {{ $patient->patientSocialHistories->first()->doctor->name ?? 'N/A' }}
                                        @else
                                            N/A
                                        @endif
                                    </td>
                                    ...
                                    <td class="responsive-width">
                                        @if ($patient->patientSocialHistories->isNotEmpty())
                                            <a href="{{ route('patient-social-histories.show', $patient->patientSocialHistories->first()) }}"
                                        ...
                                        @if ($patient->patientSocialHistories->isNotEmpty())
                                            @can('patient-social-histories-update')
                                                <a href="{{ route('patient-social-histories.edit', $patient->patientSocialHistories->first()) }}"
                                                    class="responsive-width-item btn btn-info btn-outline btn-circle btn-lg"
