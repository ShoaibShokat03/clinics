                    ...
                    <div class="col-12">
                        <div class="invoice">
                            <div class="clinic-header">
                                <h4 style="font-size: 12px;margin-bottom:10px;">{{ $ApplicationSetting->item_name ?? 'Company Name' }}</h4>
                            </div>
                            <div class="patient-info">
                                <table>
                                    <tr>
                                        <td><strong>Patient</strong> {{ $patient->name }}</td>
                                        <td><strong>MRN#</strong> {{ $patientDetail->mrn_number ?? '' }}</td>
                                        <td class="gender"><strong>Gender</strong> {{ $invoice->user->gender ?? '' }}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Date</strong> {{ date('d M Y', strtotime($invoice->invoice_date)) }}</td>
                                        <td><strong>Invoice#</strong> {{ str_pad($invoice->id, 6, '0', STR_PAD_LEFT) }}</td>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td style="text-align: left;">
                                                    {{ $invoiceItem->description ?? ($invoiceItem->patienttreatmentplanprocedures->procedure->description ?? '-') }}
                                                </td>
                                                <td>
                                                    {{ number_format($invoiceItem->price, 0) }}
                                                </td>
                                                <td>
                                                    {{ $invoiceItem->quantity }}
                                                </td>
                                                <td>
                                                    {{ number_format($invoiceItem->price * $invoiceItem->quantity, 0) }}
                                                </td>
                                            </tr>
                                            @endforeach
                                            ...
                                            <tr style="background-color: #d3d3d35c;">
                                                <td colspan="4" style="text-align: right;"><b>@lang('Sub Total'): {{ number_format($invoice->total, 0) }}</b></td>
                                            </tr>
                                            <tr style="background-color: #d3d3d35c;">
                                                <td colspan="4" style="text-align: right;"><b>@lang('Grand Total'): {{ number_format($invoice->grand_total, 0) }}</b></td>
                                            </tr>
                                            <tr style="background-color: #d3d3d35c;">
                                                <td colspan="4" style="text-align: right;"><b>@lang('Paid Amount'): {{ number_format($invoice->paid, 0) }}</b></td>
                                            </tr>
                                            @if ($invoice->due != 0)
                                            <tr style="background-color: #d3d3d35c;">
                                                <td colspan="4" style="text-align: right;"><b>@lang('Balance Amount'): {{ number_format($invoice->due, 0) }}</b></td>
                                            </tr>
                                            @endif
                                        </tbody>
                                    </table>
