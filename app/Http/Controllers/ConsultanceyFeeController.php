...
class ConsultanceyFeeController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if ($consultanceyFee) {
            ...
            if (!$company) {
                return redirect()->route('consultancey-fees.index')->with('error', trans('Company not found'));
            }
    ...
    public function store(Request $request)
    {
        ...
        $consultancyFee = ConsultanceyFee::create($data);
        $msg = "Your consultancy fee of {$consultancyFee->amount} has been recorded.";
        NotificationService::sendToUser(
            $consultancyFee->user_id,   // 👈 yeh usi user ko milega jiska consultancy fee hai
            'Consultancy Fee Added',
            $msg,
            ['consultancy_fee_id' => $consultancyFee->id]
        );
