...
class PatientTreatmentPlanController extends Controller
{
    ...
    public function index(Request $request, $doctorId = null)
    {
        ...
        $patientsinfo = PatientDetail::all();
        $doctors = DoctorDetail::all();

        // Pass the data to the view
        return view('patient-treatment-plans.index', compact('patientTreatmentPlans', 'patientsinfo', 'doctors'));
    }
    ...
    private function filter(Request $request)
    {
        ...
        if ($request->filled('patient_id')) {
            $query->whereHas('patient', function ($q) use ($request) {
                $q->where('users.id', $request->patient_id);
            });
        }
        if ($request->filled('doctor_id')) {
            $query->whereHas('doctor', function ($q) use ($request) {
                $q->where('users.id', $request->doctor_id);
            });
        }
    ...
    public function create(Request $request)
    {
        ...
        if ($request->examination_id) {
            ...
            // dd($examinationData);
            return view('patient-treatment-plans.create', compact('examinationData', 'doctors'));
        }
        ...
        return view('patient-treatment-plans.create', compact('patients', 'doctors', 'selectedPatientId'));
    }
    ...
    public function show(Request $request, PatientTreatmentPlan $patientTreatmentPlan)
    {
        ...
        // New Layout & Settings Synchronization
        $pageSetting = \App\Models\PageSetting::where('page_name', 'treatment_plan_show')->first();
        $pageSettings = $pageSetting ? $pageSetting->settings : [];
        $ApplicationSetting = \App\Models\ApplicationSetting::first();

        return view('patient-treatment-plans.show', compact('patientTreatmentPlan', 'company', 'patientTreatmentPlanProcedures', 'teeth', 'totalPrice', 'allTeethProcedures', 'pageSettings', 'ApplicationSetting'));
    }
    ...
    //     if (!$company) {
    //         return redirect()->route('patient-treatment-plans.index')->with('error', trans('Company not found'));
    //     }
    ...
    public function edit(PatientTreatmentPlan $patientTreatmentPlan)
    {
        ...
        $patientNotes = TreatmentPlanNotes::where('patient_treatment_plan_id', $patientTreatmentPlan->id)->orderBy('id', 'desc')->get();
    ...
    public function treatmentNotes(Request $request)
    {
        ...
        $notes->patient_treatment_plan_procedure_id = $request->plan_procedure_id;
        $notes->tooth_number = $request->tooth_number;
        ...
        return response()->json([
            ...
            'procedure_id' => $request->plan_procedure_id,
