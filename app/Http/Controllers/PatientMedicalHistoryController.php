...
class PatientMedicalHistoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('patient-medical-histories.index', compact('patients', 'doctors', 'patientsinfo'));
    }
    ...
    private function filter(Request $request)
    {
        $query = User::role('Patient')
            ->where('status', '1')
            ->whereHas('patientMedicalHistories', function ($query) use ($request) {
    ...
    {
        $existingHistory = PatientMedicalHistory::where('patient_id', $userid)->first();
        if ($existingHistory) {
            return redirect()->route('patient-medical-histories.edit', $existingHistory->id);
        }
