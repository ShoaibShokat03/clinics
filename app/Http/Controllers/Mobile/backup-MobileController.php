...
class MobileController extends Controller
{
                ...
                if ($doctor->user->status == 1) {
                    $doctorData[] = [
                        ...
                        'details' => [
                            ...
                            'availability' => $doctor->user->status == 1 ? "Available" : "Not Available",
    ...
    public function TestNotification()
    {
        try {
            ...
            $fc->sendToDevice(
                $request->device_token ?? 'default_token',
                'Login Successful',
                'You have successfully logged in',
                [] // Optional payload
            );
        } catch (Exception $e) {
            // Handle Firebase notification failure
            $messages = "Login successful, but failed to send notification: " . $e->getMessage();
            Storage::disk('local')->append('sendTest.log', $messages);
        }
    }
        ...
        try {
            ...
            $appointment = $patientAppointments->map(function ($appointment) {
                return [
                    ...
                    'doctor_name'  => $appointment->doctor->name,
                    'jappointment_number' => $appointment->appointment_number,
                    'appointment_date' => $appointment->appointment_date." ".$appointment->start_time,
                    'appointment_time'=>$appointment->start_time."-".$appointment->end_time,
                    'problem' => $appointment->problem
                ];
            });
    ...
    public function contact(Request $request)
    {
        ...
        try {
            ...
            if (!$settings) {
                return response()->json([
                    'status' => 'error',
                    'message ' => 'no data exist here'
                ]);
            }
            $data = [
                'description' => $settings['general.company_description'],
                'email' => $settings['general.company_email'],
