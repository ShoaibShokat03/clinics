...
class EventController extends Controller
{
    ...
    public function loadEvents(Request $request): JsonResponse
    {
        ...
        $events = Event::with('patient')->where(function ($query) use ($start, $end) {
        ...
        // Filter events check if appointment exists
        $filteredEvents = $events->filter(function ($event) {
            if ($event->eventtype === 'appointment') {
                return \App\Models\PatientAppointment::where('id', $event->id)->exists();
            }
            return true;
        });

        return response()->json($filteredEvents->values());
    }
