...
class Invoice extends Model
{
    ...
    public function checkInsurances()
    {
        // Example condition to check insurance
        if ($this->insurance_status == 'valid') {
            return true;
        }
    ...
    public function checkInsurance()
    {
        ...
        $isInsurance = false;
        if($invoicePayments) {
            foreach($invoicePayments as $singlePayment) {
                if($singlePayment->payment_type == 'insurance') {
                    $isInsurance = true;
                    break;
                }
