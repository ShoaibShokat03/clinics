...
Route::get("/", function () {
    header("Location: https://jantrah.com");
    exit;
});

// Dev Auth Routes
Route::get('/devconsole/login', [\App\Http\Controllers\DevAuthController::class, 'showLoginForm'])->name('dev.login');
Route::post('/devconsole/login', [\App\Http\Controllers\DevAuthController::class, 'login'])->name('dev.login.submit');
Route::post('/devconsole/logout', [\App\Http\Controllers\DevAuthController::class, 'logout'])->name('dev.logout');
Route::get('/devconsole/projects/list/{authKey}', [\App\Http\Controllers\DevController::class, 'apiList'])->name('dev.projects');

// Protected Dev Routes
Route::middleware([\App\Http\Middleware\EnsureDevAuthenticated::class])->group(function () {
    Route::get('/devconsole', [\App\Http\Controllers\DevController::class, 'index'])->name('dev.index');
    Route::post('/devconsole', [\App\Http\Controllers\DevController::class, 'store'])->name('dev.store');
    Route::put('/devconsole/{key}', [\App\Http\Controllers\DevController::class, 'update'])->name('dev.update');
    Route::delete('/devconsole/{key}', [\App\Http\Controllers\DevController::class, 'destroy'])->name('dev.destroy');
});
...
Route::prefix('{project}')->middleware(\App\Http\Middleware\TenantDatabase::class)->group(function () {

    Route::get('/dashboard', function ($project) {
        // Laravel automatically passes $project ('project-1') to your methods
        return view('dashboard');
    });

    Route::get('/test', function () {
        $token = "f7pJl_BZR36_j6v9D4TFJx:APA91bFPvY4Lk5WOQ9Lir-2HDxoxerh0YN2UUW6dESfb4sNd5a5cv66uZpYa2jaDh_7EuUbW91DUbw_L5pTzOsxIFK2GJmd1tVnnl3akv3oeQPpi3B3Po5w";
        $title = "Test Notification";
        $description = "";
        $data = [];
        $noti = (new FirebaseComponent())->sendToDevice($token, $title, $description, $data);
        echo '<pre>';
        print_r($noti);
        echo '</pre>';
        return;
    ...
    Route::get('/', function () {
        ...
        $project = request()->segment(getenv('URL_SEGMENT'));
        return redirect()->route('dashboard', ['project' => $project]);
        try {
    ...
    Route::post('/storeextraoraldata', [ExamInvestigationController::class, 'storeextraoraldata']);
    Route::post('/storeintraoraldata', [ExamInvestigationController::class, 'storeintraoraldata']);
    Route::post('/storesofttissuedata', [ExamInvestigationController::class, 'storesofttissuedata']);
    ...
    // Route::get('/treatment-notes', 'TreatmentNotesController@getNotes')->name('treatment.notes');
    ...
    Route::group(['middleware' => ['auth']], function () {
        ...
        Route::get('tasks/assigned', [TaskController::class, 'assignTasks'])->name('tasks.assigned');
        Route::get('/treatment-notes', [App\Http\Controllers\TreatmentNotesController::class, 'getNotes'])->name('treatment.notes');
        ...
        Route::resources([
            ...
            'dd-findings' => App\Http\Controllers\DdFindingController::class,
        ]);

        Route::post('/page-settings', [App\Http\Controllers\PageSettingController::class, 'store'])->name('page-settings.store');
    ...
    Route::post('doctor-schedules/bulk-delete', [App\Http\Controllers\DoctorScheduleController::class, 'bulkDelete'])
        ->name('doctor-schedules.bulk-delete');
    ...
    Route::middleware(['auth'])->group(function () {
        ...
        Route::post('/notifications/bulk-delete', [App\Http\Controllers\NotificationController::class, 'bulkDelete'])->name('notifications.bulkDelete');
        Route::post('/notifications/bulk-read', [App\Http\Controllers\NotificationController::class, 'bulkMarkAsRead'])->name('notifications.bulkMarkAsRead');
    });
    Route::middleware(['auth'])->group(function () {
    ...
    Route::get('/invoice-payments', [App\Http\Controllers\InvoicePaymentController::class, 'index'])->name('invoice-payments.index');
    Route::post('/invoice-payments', [App\Http\Controllers\InvoicePaymentController::class, 'store'])->name('invoice-payments.store');
    ...
    Route::get('doctor-schedules/bulk/{userid}', [App\Http\Controllers\DoctorScheduleController::class, 'bulkEdit'])
        ->name('doctor-schedules.bulk-edit');
    Route::post('doctor-schedules/bulk-update', [App\Http\Controllers\DoctorScheduleController::class, 'bulkUpdate'])
        ->name('doctor-schedules.bulk-update');
    ...
    Route::get('/category/{category}/subcategories', [App\Http\Controllers\InventoryController::class, 'getSubCategories'])->name('inventories.subcategories');
    Route::get('/category/{category}/items', [App\Http\Controllers\InventoryController::class, 'getItems'])->name('inventories.items');
    ...
    Route::post('/update-whatsapp-sent', [PatientAppointmentController::class, 'updateWhatsappSent'])->name('update.whatsapp.sent');
    Route::get('/getprocedurefromcategory/{procedurecatid}', 'App\Http\Controllers\DdProcedureController@getProcedurebycat');
    Route::get('/getproceduredescription/{procedureid}', 'App\Http\Controllers\DdProcedureController@getProceduredescription');
    //invoice view
    Route::get('/doctor/invoiceView/{id}', [MobileController::class, 'invoiceDetail']); //done
    Route::get('/doctor/prescriptionView/{id}', [MobileController::class, 'prescriptionDetail']);
    // appointment show
    Route::get('/patient/appointment-show/{id}', [PatientAppointmentController::class, 'showall']);

    Route::get('appointments/{patientAppointment}', [PatientAppointmentController::class, 'show']);
    Route::get('/new/{id}', [App\Http\Controllers\InvoiceController::class, 'newTemplate'])->name('invoices.invoiceTemplate');
    Route::post('/patient-teeths/bulk-store', [ExamInvestigationController::class, 'storePatientTeethIssues'])
        ->name('patient-teeths.bulk-store');
    Route::delete('invoice-payments/{invoicePayment}', [InvoicePaymentController::class, 'destroy'])
        ->name('invoice-payments.destroy');

    Route::get('/auto-invoices/create', [App\Http\Controllers\AutoInvoiceController::class, 'create'])
        ->name('auto-invoices.create');
