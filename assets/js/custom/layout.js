$(document).ready(function () {
    ...
    window.loadFiles = function (recordId) { // Attach to window object
        ...
        if (typeof getFilesUrl === 'undefined') {
            console.warn('getFilesUrl is not defined. Skipping file load.');
            return;
        }
    ...
    $(document).on('change', 'input[type="file"]', function () {
        ...
        var examinationId = $('#hidden_examination_id').val(); // Get tooth number from modal ..
        ...
        if (typeof uploadFilesUrl === 'undefined') {
            // alert('uploadFilesUrl is not defined. Cannot upload files.');
            return;
        }

        //uploading Text
        const uploadingText = document.createElement("div");
        uploadingText.id = "uploadingText";
        uploadingText.setAttribute("style", "width:fit-content;padding:8px 16px;background:#4c4c4c;color:#fff;font-size:1.2rem;border-radius:5px;margin:0px auto;postion:absolute;z-index:9999;");
        uploadingText.innerHTML = "Uploading...";

        //  append it after uploading input field
        $(this).parent().append(uploadingText);
        ...
        $.ajax({
            ...
            complete: function () {
                document.querySelector("#uploadingText")?.remove();
            }
        });
    });
